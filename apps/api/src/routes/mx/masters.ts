...
export async function registerMxMastersRoutes(app: FastifyInstance, opts: { db: Db }) {
  ...
  // Addresses
  app.get("/mx/addresses", async () => ({ data: db.prepare(`SELECT * FROM mx_delivery_addresses WHERE deleted_at IS NULL ORDER BY party_name`).all() }));
  app.post("/mx/addresses", async (req) => {
    ...
    const id = Number(
      db.prepare(`INSERT INTO mx_delivery_addresses(party_name, address_line, city, state) VALUES (?,?,?,?)`)
        .run(body.party_name, body.address_line ?? null, body.city ?? null, body.state ?? null).lastInsertRowid,
    );
    return { data: { id, ...body } };
  });
